...
session::session(session&& rhs):
	...
	query_stream_(std::move(rhs.query_stream_)),
	query_transformation_(rhs.query_transformation_),
	logStream_(rhs.logStream_),
	lastQuery_(std::move(rhs.lastQuery_)),
	lastConnectParameters_(rhs.lastConnectParameters_),
	uppercaseColumnNames_(rhs.uppercaseColumnNames_),
	backEnd_(rhs.backEnd_),
	gotData_(rhs.gotData_),
	isFromPool_(rhs.isFromPool_),
	poolPosition_(rhs.poolPosition_),
	pool_(rhs.pool_)
{
	rhs.query_transformation_ = NULL;
	rhs.logStream_ = NULL;
	rhs.backEnd_ = NULL;
	rhs.isFromPool_ = false;
	rhs.pool_ = NULL;
}
