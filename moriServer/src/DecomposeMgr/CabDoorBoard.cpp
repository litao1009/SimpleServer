...
template<>
class	DecomposeTable<DCP_CABDOOR_BOARD_TYPE>::Imp
{
public:
...
template<>
void	DecomposeTable<DCP_CABDOOR_BOARD_TYPE>::PreProcess()
{
	auto& imp_ = *ImpSPtr_;
...
#pragma message("TODO")
	...
	auto& pntCnt = InfoSPtr_->CabDoor_;
	auto pntItor = pntCnt.find(imp_.CabDoorID_);
	if ( pntItor == pntCnt.end() )
	{
		LOG_ERROR << L"CabDoor_ are supposed to have ID:" << imp_.CabDoorID_;
...
template<>
void	DecomposeTable<DCP_CABDOOR_BOARD_TYPE>::PostProcess()
{
	auto& imp_ = *ImpSPtr_;

	InfoSPtr_->CabDoorBoard_.emplace(std::piecewise_construct,
