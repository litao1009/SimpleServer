...
template<>
class	DecomposeTable<DCP_CABDOOR_HANDLE_TYPE>::Imp
{
public:
...
template<>
void	DecomposeTable<DCP_CABDOOR_HANDLE_TYPE>::Init(Statement& ss)
{
	...
	ss.Select(
		...
		.InnerJoin(GC_CabDoorRegionInfo).On(GC_CabDoorRegionInfo.CabDoorRegionID == GC_CabDoorHandleInfo.CabParentID &&
		GC_CabDoorHandleInfo.VerCode == GC_CabDoorRegionInfo.VerCode &&
		GC_CabDoorHandleInfo.FactoryID == GC_CabDoorRegionInfo.FactoryID &&
		GC_CabDoorHandleInfo.OrderID == GC_CabDoorRegionInfo.OrderID &&
		GC_CabDoorHandleInfo.ParentType==1)
...
template<>
void	DecomposeTable<DCP_CABDOOR_HANDLE_TYPE>::PreProcess()
{
	auto& imp_ = *ImpSPtr_;
...
template<>
void	DecomposeTable<DCP_CABDOOR_HANDLE_TYPE>::PostProcess()
{
	auto& imp_ = *ImpSPtr_;
...
template<>
void	DecomposeTable<DCP_CABDOOR_HANDLE_TYPE>::PostDecompose(soci::session& sql, soci::session& sqlInsert)
{
